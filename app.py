...
initial_cost = strategy.price(spot, T, r, sigma)
S_range = np.linspace(spot * 0.8, spot * 1.2, 200)
payoff = main.simulate_pnl_exipiration(strategy, S_range, initial_cost)
real_pnl = main.simulate_pnl(strategy, S_range, T, r, sigma, initial_cost)

# Display P&L
col1, col2 = st.columns(2)
col1.subheader("P&L Chart")
fig_pl = main.plot_results(S_range, payoff, real_pnl, spot)
col1.plotly_chart(fig_pl, use_container_width= False)

# Disply 3D surface plot of strategy value
col2.subheader("3D surface plot of strategy value")
